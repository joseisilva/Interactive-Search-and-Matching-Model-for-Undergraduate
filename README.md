...
The package contains a self-contained teaching paper, an interactive
browser-based simulator, lecture slides, a Google Colab notebook, and the
source code that generates all the figures in the paper.

> **➜ Live interactive page:**
> [**An Interactive Introduction to Search and Matching Models**](https://joseisilva.github.io/Interactive-Search-and-Matching-Model-for-Undergraduate/)
>
> The full paper, all figures, and a live simulator — runs entirely in your
> browser, no installation needed. The same content is also available offline
> via [`index.html`](index.html) in this package.
...
├── README.md                              <-- this file
├── index.html                             <-- interactive teaching webpage (offline copy)
...
The fastest way to explore the material is the live page hosted on GitHub
Pages:

➜ **<https://joseisilva.github.io/Interactive-Search-and-Matching-Model-for-Undergraduate/>**

It contains the full paper in browser-readable form, all five figures, and a
**live simulator** that runs the model directly in your browser via plain
JavaScript — no Python installation, no internet connection needed after the
page loads. You can also share this URL with students, embed it in a syllabus,
or link to it from a course management system.

If you'd rather work offline, [`index.html`](index.html) in this package is
the same file. Just double-click it to open in any recent Chrome, Firefox, or
Safari.

### Read the paper

Open `search_matching_teaching.pdf`. It is a teaching paper organized as:

- **Body (Sections 1–14):** environment, matching function, firms, JC, wage
  ...
  connections to the literature, what the model leaves out, conclusion.
- **Appendix A:** derivation of the job creation condition from the firm's
  steady-state reduced-form profit-maximization problem.
...
`slides.pdf` is a Beamer deck (16:9 aspect ratio) that mirrors the paper and
uses the same five figures. Suitable for a 50-minute lecture. To recompile
after edits:
...
2. Click **File → Upload notebook** and pick `search_matching_simulator.ipynb`.
   (Use *Upload notebook*, not *Open file* or `!python file.ipynb` — the
   `.ipynb` is JSON, not a Python script.)
...
sits on the **Beveridge curve**, whose live U.S. analog is published by the
Bureau of Labor Statistics
[here](https://www.bls.gov/charts/job-openings-and-labor-turnover/job-openings-unemployment-beveridge-curve.htm).
